                ...
                <form action="{{ route('pembayaran.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <select name="pinjam_id"
                                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                                <option value="" disabled selected>-- Pilih Fasilitas yang Dipinjam --</option>
                                @forelse ($peminjaman as $p)
                                    <option value="{{ $p->pinjam_id }}">
                                        {{ $p->fasilitas->nama ?? 'Fasilitas #' . $p->pinjam_id }} -
                                        {{ $p->warga->nama ?? 'Umum' }}
                                    </option>
                                @empty
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                        <a href="{{ route('pembayaran.index') }}"
                            class="text-gray-400 hover:text-gray-600 font-medium px-4">Batal</a>
                        <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-2.5 rounded-xl shadow-lg shadow-blue-100 transition-all">
                            Simpan Pembayaran
                        </button>
                    </div>
                </form>
