    ...
    <section class="py-16">
        <div class="container mx-auto px-6 max-w-3xl">
            <div class="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                ...
                <form action="{{ route('pinjam.update', $data->pinjam_id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-5">
                        ...
                        <select name="warga_id"
                            ...
                            @foreach ($warga as $item)
                                <option value="{{ $item->id }}" {{ $data->warga_id == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="tujuan" rows="3"
                            class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none" required>{{ $data->tujuan }}</textarea>
                    </div>

                    {{-- Total Biaya --}}
                    <div class="mb-5">
                        <label class="block font-semibold text-gray-700 mb-2">Total Biaya</label>
                        <input type="number" name="total_biaya" value="{{ $data->total_biaya }}"
                            class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none"
                            required>
                    </div>
