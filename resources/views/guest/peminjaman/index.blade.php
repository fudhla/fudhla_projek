    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($peminjamans as $item)
            <div class="bg-white shadow-md rounded-lg p-5 border border-gray-200">
                @php
                    $fotoMedia = $item->media->first();
                    $fotoFasilitas = $item->fasilitas->foto ?? null;
                @endphp

                <img src="{{ $fotoMedia ? asset('storage/' . $fotoMedia->file_url) : ($fotoFasilitas ? asset('storage/' . $fotoFasilitas) : asset('assets/images/default.jpg')) }}"
                    class="w-full h-40 object-cover rounded-lg mb-4 border">

                <p class="text-gray-700 text-sm mb-1">
                    <span class="font-semibold">Fasilitas:</span> {{ $item->fasilitas->nama ?? '-' }}
                </p>

                <p class="text-gray-700 text-sm mb-1">
                    <span class="font-semibold">Jenis:</span> {{ $item->fasilitas->jenis ?? '-' }}
                </p>

                <p class="text-gray-700 text-sm mb-1">
                    <span class="font-semibold">Alamat:</span> {{ $item->fasilitas->alamat ?? '-' }}
                </p>

                <p class="text-gray-700 text-sm mb-1">
                    <span class="font-semibold">Kapasitas:</span> {{ $item->fasilitas->kapasitas ?? '-' }} orang
                </p>

                <p class="text-gray-700 text-sm mb-2">
                    <span class="font-semibold">Deskripsi:</span> {{ $item->fasilitas->deskripsi ?? '-' }}
                </p>

                <p class="text-gray-600 text-sm mb-1">
                    <span class="font-medium">Dipinjam oleh:</span> {{ $item->warga->nama ?? '-' }}
                </p>
