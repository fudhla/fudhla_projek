    ...
    <div class="w-full max-w-2xl">
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
            ...
            <div class="bg-indigo-600 p-6 text-white">
                <h3 class="text-xl font-bold">Edit Petugas Fasilitas</h3>
                <p class="text-indigo-100 text-sm">Perbarui informasi petugas fasilitas lingkungan.</p>
            </div>

            <form action="{{ route('petugas.update', $petugas->petugas_id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div class="relative">
                            <select name="fasilitas_id" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all appearance-none cursor-pointer">
                                ...
                                @foreach ($fasilitas as $f)
                                    <option value="{{ $f->fasilitas_id }}" {{ $f->fasilitas_id == $petugas->fasilitas_id ? 'selected' : '' }}>
                                        {{ $f->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="petugas_warga_id" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all appearance-none cursor-pointer">
                                ...
                                @foreach ($warga as $w)
                                    <option value="{{ $w->id }}" {{ $w->id == $petugas->petugas_warga_id ? 'selected' : '' }}>
                                        {{ $w->nama }}
                                    </option>
                                @endforeach
                            </select>
